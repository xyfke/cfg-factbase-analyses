...
def verify_cypher_files(check_cfg, cypher_name, cypher_folder, classification, 
                            phase_n = None):
    """
    ------------------------------------------------------------------------
    ...
    ------------------------------------------------------------------------
    Parameters:
        check_cfg - whether or not query performs CFG validation (string)
        cypher_name - the name of cypher that we are about to run (string)
        cypher_folder - directory storing all the cypher files (string)
        classification - general classification of query (string)
        phase_n - the phase number (int, default: None)
    Returns:
        cypher files are correct (boolean)
    --------------------------------------------------------------------------
    """
    query_dir = "{}/{}/{}/{}/{}".format(cypher_folder, classification, cypher_type, check_cfg, phase_n)

    return os.path.exists(query_dir), query_dir
