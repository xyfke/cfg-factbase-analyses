...
#   1. Creates a neo4j output direcotry: ~/queryResults/testOutput/
#   2. Creates a neo4j instance directory: ~/neo4jInstances/n-test/
#   3. Creates a json file with neo4j instance and factbase information: 
#           ~/neo4jscripts/fileCypher/neo4jCypher/script-json/test.json
#
# Assumptions:
#   1. All factbases are stored in the factbases directory inside this gitlab project
#   2. Inside each respective software factbase folder, there is a components.csv file
#   3. Inside each factbase, there are different versions labeled by date

read -p "Enter software name: " software
[ -z "$software" ] && echo "Need to provide a software name." && exit 0

read -p "Enter factbase location(default: factbases folder): " factbaseLocation
if [ -z "$factbaseLocation" ]; then
    read -p "Factbase Date (mm-dd): " date
    [ -z "$date" ] && echo "Need to provide a date." && exit 0
    neo4jFactPath="${HOME}/neo4jscripts/factbases/${software}/facts_${date}/"
    componentPath="${HOME}/neo4jscripts/factbases/${software}/components.csv"
else
    neo4jFactPath="${factbaseLocation}"
    componentPath="${factbaseLocation}components.csv"
fi
...
# Get APOC project
neo4jApocProject="${HOME}/neo4j-apoc-CFG-SE"
if [ ! -d ${neo4jApocProject} ]; then
    git clone https://github.com/xyfke/neo4j-apoc-CFG-SE.git "${HOME}/neo4j-apoc-CFG-SE/"
fi

# Download Neo4J instance
neo4jTar="${HOME}/neo4j-community-4.4.8-unix.tar.gz"
if [ ! -f ${neo4jTar} ]; then
    wget http://www.neo4j.com/customer/download/neo4j-community-4.4.8-unix.tar.gz -P "${HOME}"
fi

# tar the neo4j instance and create the file
if [ ! -d "${HOME}/neo4jInstances" ]; then
    mkdir -p "${HOME}/neo4jInstances"
fi

neo4jLocation="${HOME}/neo4jInstances/n-${software}/"
if [ ! -d ${neo4jLocation} ]; then 
    tar xf ${neo4jTar} -C "${HOME}"
    mv "${HOME}/neo4j-community-4.4.8/" "${neo4jLocation}"
fi;
...
cd "${neo4jApocProject}"
git checkout fk_backward_integration
git pull
./gradlew shadow
...
neo4joutput="${HOME}/queryResults/${software}Output/"
if [ ! -d ${neo4joutput} ]; then
    mkdir -p ${neo4joutput}
fi

jsonFilePath="${HOME}/neo4jscripts/fileCypher/neo4jCypher/script-json/${software}.json"
